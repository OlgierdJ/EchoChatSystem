...
using Echo.Domain.EntityFrameworkCore.EFCORE.ValueGenerators;
...
public class ChatInviteConfiguration : IEntityTypeConfiguration<ChatInvite>
{
    public void Configure(EntityTypeBuilder<ChatInvite> builder)
    {
        ...
        builder
           ...
           .IsRequired();
        builder.HasOne(b => b.Inviter).WithMany(e => e.ChatInvites).HasForeignKey(b => b.InviterId).OnDelete(DeleteBehavior.Restrict).IsRequired(); //maybe check if invites are automatically deleted when the one who creates it leaves a chat or server.
        builder.HasOne(b => b.Subject).WithMany(b => b.Invites).HasForeignKey(b => b.SubjectId).OnDelete(DeleteBehavior.Cascade).IsRequired();
    }
}
